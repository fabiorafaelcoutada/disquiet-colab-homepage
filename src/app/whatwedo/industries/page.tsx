...
import { IndustriesFrontmatter, IndustrySector, ApplicationDetail } from '@/types/industry';
...
const getSectorIcon = (iconName: string) => {
    switch (iconName.toLowerCase()) {
        case 'car': return '🚗';
        case 'factory': return '🏭';
        case 'heart': return '⚕️'; // Rod of Asclepius/Medical
        case 'home': return '🏠';
        default: return '🌎';
    }
};
...
export default async function IndustriesPage() {
    ...
    const { data, contentHtml } = await getMarkdownContent('pages/industries');
    const pageData = data as IndustriesFrontmatter;
        ...
        <div className="flex-grow flex flex-col w-full">
            ...
            <section className="w-full py-20 px-6 bg-white dark:bg-gray-900">
                <div className="max-w-6xl mx-auto space-y-16">
                    ...
                    {pageData.industrySectors.map((sector: IndustrySector, index: number) => (
                        <div
                            key={index}
                            className="grid grid-cols-1 lg:grid-cols-3 gap-10 border-b pb-8 dark:border-gray-700 last:border-b-0"
                        >
                            {/* LEFT COLUMN: Sector Header/Description */}
                            ...
                            {/* RIGHT COLUMN: Detailed Applications List */}
                            <div className="lg:col-span-2 space-y-6">
                                {sector.applications.map((app: ApplicationDetail, appIndex: number) => (
                                    <div key={appIndex} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
